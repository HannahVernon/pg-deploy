...
public sealed class SchemaDiffer
{
    ...
    private static string FormatCascadeWarning(string baseWarning, List<string> dependents)
    {
        if (dependents.Count == 0)
            return $"{baseWarning} No dependent objects detected.";
    ...
    private void DiffFunctions(List<SchemaChange> changes)
    {
        ...
        if (_allowDrops)
        {
            var emittedDrops = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (var (key, tgt) in _target.Functions)
            {
                if (!_source.Functions.ContainsKey(key))
                {
                    ...
                    var dropSig = tgt.ParameterSignature != null
                        ? $"{tgt.QualifiedName}({tgt.ParameterSignature})"
                        : tgt.QualifiedName;
                    var dropSql = $"DROP {dropKw} IF EXISTS {dropSig};";

                    if (emittedDrops.Add(dropSql))
                    {
                        changes.Add(new SchemaChange
                        {
                            Category = ChangeCategory.Function,
